        ...
        foreach ($dataType->ajaxList() as $dataRow) {
            $query->editColumn($dataRow->field, function($dataTypeContent) use($request, $slug, $dataRow){
                ...
                if($dataRow->details) {
                    ...
                    if($options && isset($options->relationship)) {
                        $dataTypeId = DataType::find($dataRow->data_type_id);
                        if(isset($options->relationship->modelRelationName)) {
                            $model = $options->relationship->modelRelationName;
                            $model = str_replace('/', '\\', $model);
                            if(class_exists($model)) {
                                $model = app($model);
                                $value = $model::find($dataTypeContent->{$dataRow->field});
                                if($value) {
                                    $content = $value->name;
                                }
