@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, camel_case($row->field) ) )
        <p class="label label-warning"><i class="admin-warning"></i> {{ __('admin.form.field_select_dd_relationship', ['method' => camel_case($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, camel_case($row->field) ) )
        ...
        @if(isset(request()->crud_id))
            @php
                $selected_value = request()->crud_id;
            @endphp
        @endif

        <select class="form-control select2"  data-name="{{ $row->display_name }}" name="{{ $row->field }}" @if(isset($customParameter)) onchange="changeCategory(this);" @endif>
            <?php $default = (isset($options->default) && !isset($dataTypeContent->{$row->field})) ? $options->default : null; ?>

            @if(isset($options->options))
                <optgroup label="{{ __('admin.generic.custom') }}">
